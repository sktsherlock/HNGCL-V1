...
def train(drop_weights1, weight, semi, batch):
    ADNet.requires_grad_(False)
    model.requires_grad_(True)
    model.train()
    model_optimizer.zero_grad()
    ...
    loss = model.loss_neg(z1, z2, z3, batch_size= batch, weight=weight, semi= semi)
...
def train_hard(drop_weights1, weight, semi, batch, AD_True: int, AD_hard: int, SE: int, True_gap=1, False_gap=1):
    def drop_edge(idx: int, edge_index, drop_weights):
        if config['drop_scheme'] == 'uniform':
            return dropout_adj(edge_index, p=config[f'drop_edge_rate_{idx}'])[0]
        elif config['drop_scheme'] in ['degree', 'evc', 'pr']:
    ...
    for i in range(AD_hard):  # 使生成的真实的样本 变得困难
        ...
        z3 = ADNet(x_1, edge_index_1)
        z3 = ADNet.Generate_hard(z1, z3)
        loss = - model.loss_neg(z1, z2, z3, batch_size= batch, weight=weight, semi=semi)  # + Dis(discriminator, z1, z3) #困难 且 真实;
        loss.backward(retain_graph=True)
        ADNet_optimizer.step()
    ...
    with open(result_file, 'a+', encoding='utf-8') as fb:
        ...
        if config['hard'] == True:
            if config['warmup'] == True:
                ...
                loss_hard = train_hard(drop_weights1, config['weight'], config['semi'], config['batch'], config['AD_True'], config['AD_hard'], config['SE'], config['True_gap'], config['False_gap'])
                print('warmup phase final loss:', loss_hard)

            for epoch in range(config["num_epochs"]+1):
                if config['mode'] == 'normal':
                    loss = train(drop_weights1, config['weight'], config['semi'], config['batch'])
                    if epoch < config['stop']:  # 参数
                        _ = train_hard(drop_weights1, config['weight'], config['semi'], config['batch'], 1, 1, 1)
